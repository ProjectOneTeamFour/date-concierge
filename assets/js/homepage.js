...
var performValidation = function()
{
    ...
    if ( userName === "" ) {
        errMsg += "<li>Whoa. Slow down dater. We need to know your name first. Please enter it.</li>";
    ...
    if (!Date.parse(datecheck)){
        errMsg += "<li>We need to know when you're planning the big event. Please enter a valid date.</li>";
        elementList.push(dateEl);
    }

    // check if no mood option is selected
    if ( mood === "" ) {
        errMsg += "<li>We do our best work when we know what mood you're trying to set. Please select one from the drop down.</li>";
...
var getCityCoordinates = function(country)
{
    ...
    if(country === '')
    {
        showMessage("We need to know where you are. Please enter a city or let us use your location.");
    }
    else
    {
        fetch(geocoderURL)
